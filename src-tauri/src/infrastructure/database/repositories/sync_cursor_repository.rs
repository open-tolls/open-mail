...
impl SyncCursorRepository for SqliteSyncCursorRepository {
    async fn find_by_account(&self, account_id: &str) -> Result<Vec<SyncCursor>, DomainError> {
        ...
        let mut statement = connection
            .prepare(
                "SELECT account_id, folder_id, folder_path, last_message_id, last_message_observed_at,
                        last_thread_id, observed_message_count, last_sync_started_at,
                        last_sync_finished_at, updated_at
                 FROM sync_cursors
                 WHERE account_id = ?1
        ...
        connection
            .execute(
                "INSERT INTO sync_cursors (
                    account_id, folder_id, folder_path, last_message_id, last_message_observed_at,
                    last_thread_id, observed_message_count, last_sync_started_at,
                    last_sync_finished_at, updated_at
...
fn map_sync_cursor(row: &Row<'_>) -> rusqlite::Result<SyncCursor> {
    Ok(SyncCursor {
        ...
        last_thread_id: row.get(5)?,
        observed_message_count: row.get(6)?,
