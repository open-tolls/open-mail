...
**Status atual:** quarto corte entregue, com persistencia backend, loop de processamento em background a cada `30s`, `Send later` no composer com presets + data/hora custom, auto-envio no horario, pasta pseudo `Scheduled` no shell, cancelamento do agendamento, reabertura do draft agendado no composer com anexos restaurados em memoria, notificacao desktop no envio concluido e fallback local no modo web/teste.
...
**Status atual:** segundo corte entregue no reader, com banner visual para mensagens suspeitas, deteccao local de sender spoofing, deteccao de mismatch entre texto do link e URL real, CTA de unsubscribe quando os headers `List-Unsubscribe` estao presentes e `one-click unsubscribe (RFC 8058)` funcional quando disponivel.
...
**Status atual:** fluxo local entregue end-to-end, com `Remind me` no composer, presets `1 dia`, `3 dias`, `1 semana` e custom datetime, persistencia local dos follow-ups, pasta pseudo `Reminders`, auto-cancel ao detectar resposta e wake-up da thread no inbox com notificacao desktop quando o lembrete vence.
...
- [x] Reminder cancelado automaticamente se recebe resposta
- [x] Thread movido para topo se nao respondido
- [x] Notificacao desktop
